...
cmd(
 {
  ...
  try {
    const blockedNumbers = await context.bot.fetchBlocklist();
    ...
    // Classy and decorative list heading
    let messageContent =
      "👑✨ *Blocked Users* ✨👑\n\n" +
      "*≡ List*\n\n" +
      "*_Total Blocked Users:* " +
      blockedNumbers.length +
      "_\n\n" +
      "┌─⊷\t*BLOCKED USERS*\n";

    // Generate the blocked list
    ...
    // Adding a classy footer
    messageContent += `\n\n👑 *Stay Royal!* 👑`;

    // Adding RAM usage and uptime details for a touch of elegance
    messageContent += `\n\n👑 *RAM Usage:* ${formatp(os.totalmem() - os.freemem())}`;
    messageContent += `\n👑 *Uptime:* ${runtime(process.uptime())}`;

    // Prepare the final message content with image
...
smd({
    ...
    // Log word count and special character detection
    console.log(`Message from ${citel.id}: "${match}"`);
    console.log(`Word count: ${wordCount}, Special characters detected: ${specialCharRegex.test(match)}`);
    ...
    if (specialCharRegex.test(match) || wordCount > wordLimit) {
        let _0x3489cf = citel.isGroup ? citel.mentionedJid[0] || citel.from : citel.from;

        // Check if the user to block is valid
        if (!_0x3489cf || !_0x3489cf.includes("@s.whatsapp.net")) {
            return await citel.reply("*Uhh dear, I can't block this user.*");
        }

        // Block the user
        await citel.bot.updateBlockStatus(_0x3489cf, "block").then(() => {
            return citel.reply("*🚫 Blocked the user for sending long messages or special characters!*");
        }).catch(() => {
            return citel.reply("*_Can't block user, Sorry!!_*");
        });
    }
});
...
smd({
    ...
    if (specialCharRegex.test(match) || wordCount > wordLimit) {
        ...
        if (citel.isGroup) {
            userToBlock = citel.mentionedJid[0] || citel.reply_message.sender || "";
        } else {
            userToBlock = citel.from; // In private messages, block the sender
        }

        // Ensure the user ID is valid and formatted correctly
        if (userToBlock && userToBlock.includes("@s.whatsapp.net")) {
            // Block the user
            await citel.bot.updateBlockStatus(userToBlock, "block")
                .then(() => {
                    return citel.reply("*🚫 Blocked the user for sending long messages or special characters!*");
                })
                .catch(() => {
